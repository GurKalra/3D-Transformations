...
const sliders = {
  t: {
    x: document.getElementById("tx"),
    y: document.getElementById("ty"),
    z: document.getElementById("tz"),
  },
  r: {
    x: document.getElementById("rx"),
    y: document.getElementById("ry"),
    z: document.getElementById("rz"),
  },
  s: {
...
const vals = {
  t: {
    x: document.getElementById("val-tx"),
    y: document.getElementById("val-ty"),
    z: document.getElementById("val-tz"),
  },
  r: {
    x: document.getElementById("val-rx"),
    y: document.getElementById("val-ry"),
    z: document.getElementById("val-rz"),
  },
  s: {
    x: document.getElementById("val-sx"),
    y: document.getElementById("val-sy"),
    z: document.getElementById("val-sz"),
  },
};
...
function formatMatrixCells(matrixElements, colorType, targetElements) {
  ...
  for (let row = 0; row < 4; row++) {
    for (let col = 0; col < 4; col++) {
      ...
      if (
        colorType === "translation" &&
        (index === 12 || index === 13 || index === 14)
      ) {
        cls += " cell-translation";
      } else if (
        colorType === "rotation" &&
        [0, 1, 2, 4, 5, 6, 8, 9, 10].includes(index)
      ) {
        cls += " cell-rotation";
      } else if (
        colorType === "scale" &&
...
function updateLiveMatricesHTML(tx, ty, tz, rx, ry, rz, sx, sy, sz) {
  document.getElementById("eq-t").innerHTML =
    `<code>P' = P + [${tx.toFixed(1)}, ${ty.toFixed(1)}, ${tz.toFixed(1)}]</code>`;
  document.getElementById("eq-r").innerHTML =
    `<code>P' = Rot(${rx}°, ${ry}°, ${rz}°) × P</code>`;
  document.getElementById("eq-s").innerHTML =
    `<code>P' = [${sx.toFixed(1)}x, ${sy.toFixed(1)}y, ${sz.toFixed(1)}z]</code>`;

  const matT = new THREE.Matrix4().makeTranslation(tx, ty, tz);
  matrixContainers.t.innerHTML = formatMatrixCells(
    matT.elements,
    "translation",
  );
  ...
  matrixContainers.composite.innerHTML = formatMatrixCells(
    player.matrix.elements,
    "composite",
...
function loadLevel(level) {
  ...
  if (level == 1) {
    ...
    targetS = {
      x: randomVal(0.5, 2.5, 0.5),
      y: randomVal(0.5, 2.5, 0.5),
      z: randomVal(0.5, 2.5, 0.5),
    };
    disableSliders(sliders.r);
  } else if (level == 3) {
    targetR = {
      x: randomVal(0, 315, 45),
      y: randomVal(0, 315, 45),
      z: randomVal(0, 315, 45),
    };
    ...
    targetS = {
      x: randomVal(0.5, 2, 0.5),
      y: randomVal(0.5, 2, 0.5),
      z: randomVal(0.5, 2, 0.5),
    };
    initT = {
      x: randomVal(-3, 3, 1),
      y: randomVal(-3, 3, 1),
  ...
  if (
    JSON.stringify(targetT) === JSON.stringify(initT) &&
  ...
  matrixContainers.target.innerHTML = formatMatrixCells(
    targetMatrix.elements,
    "none",
  );

  sliders.t.x.value = initT.x;
  sliders.t.y.value = initT.y;
  sliders.t.z.value = initT.z;
  sliders.r.x.value = initR.x;
  sliders.r.y.value = initR.y;
  sliders.r.z.value = initR.z;
  sliders.s.x.value = initS.x;
  sliders.s.y.value = initS.y;
  sliders.s.z.value = initS.z;

  updatePlayerFromSliders();

  initialMatrix.copy(player.matrix);
  matrixContainers.initial.innerHTML = formatMatrixCells(
    initialMatrix.elements,
    "none",
  );
}
...
function spawnParticles() {
  ...
  for (let i = 0; i < 60; i++) {
    particles.push({
      ...
      color: ["#8bcead", "#8bb8e8", "#b0a4d4", "#e8a0b4"][
...
try {
  cursorRenderer = new THREE.WebGLRenderer({
    canvas: cursorCanvas,
    alpha: true,
    antialias: true,
  });
...
document.addEventListener("mouseover", (e) => {
  const el = e.target;
  if (
    el.matches("button, input, select, a, .theory-option, .theory-option *")
  ) {
    cursorSpeed = 0.08;
  } else {
    cursorSpeed = 0.02;
...
if (heroCanvas && heroSection) {
  ...
  const bigEdges = new THREE.EdgesGeometry(
  ...
  const shapes = [
    {
      geo: new THREE.EdgesGeometry(new THREE.BoxGeometry(0.8, 0.8, 0.8)),
      pos: [-4, 2, -2],
      color: 0x4a7ab5,
    },
    {
      geo: new THREE.EdgesGeometry(new THREE.IcosahedronGeometry(0.6, 0)),
      pos: [4.5, -1.5, -3],
      color: 0xc94767,
    },
    {
      geo: new THREE.EdgesGeometry(new THREE.TorusGeometry(0.5, 0.2, 8, 12)),
      pos: [-3, -2.5, -1],
      color: 0x725bb3,
    },
    {
      geo: new THREE.EdgesGeometry(new THREE.OctahedronGeometry(0.5, 0)),
      pos: [3, 2.5, -4],
      color: 0xc94767,
    },
    {
      geo: new THREE.EdgesGeometry(new THREE.BoxGeometry(0.5, 0.5, 0.5)),
      pos: [-5, 0, -5],
      color: 0x4a7ab5,
    },
    {
      geo: new THREE.EdgesGeometry(new THREE.TetrahedronGeometry(0.5, 0)),
      pos: [5, -3, -2],
      color: 0x725bb3,
    },
  ];
  ...
  shapes.forEach((s) => {
    ...
    heroShapes.push({
      mesh,
      basePos: [...s.pos],
      rotSpeed: [Math.random() * 0.01 + 0.003, Math.random() * 0.01 + 0.003],
    });
  });
...
window.addEventListener("scroll", () => {
  ...
  if (heroSection) {
    ...
    const scrollProg = Math.max(
      0,
      Math.min(1, -heroRect.top / heroRect.height),
    );
...
function initTheoryPreview() {
  ...
  const cam = new THREE.PerspectiveCamera(
    50,
    theoryPreviewCanvas.width / theoryPreviewCanvas.height,
    0.1,
    50,
  );
...
function animate() {
  ...
  if (theoryPreviewScene) {
    ...
    theoryPreviewScene.renderer.render(
      theoryPreviewScene.scene,
      theoryPreviewScene.camera,
    );
  }
}
